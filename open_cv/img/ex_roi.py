...
ep_lg_gray_img = cv2.cvtColor(oc_lg, cv2.COLOR_BGR2GRAY)
ep_lg_gray_img = cv2.GaussianBlur(ep_lg_gray_img, (5, 5), 0)
element = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))
ep_lg_erode = cv2.erode(ep_lg_gray_img, element)


# detected_edges = cv2.GaussianBlur(ep_lg_erode, (3, 3), 0)
detected_edges = cv2.Canny(ep_lg_erode, 30, 150, apertureSize=kernel_size)
